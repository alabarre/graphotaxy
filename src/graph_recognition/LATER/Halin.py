...
from graph_recognition.misc_algo import degree_sequence  # LABARRE
...
def isK4(G):
    """Is this graph K4?"""
    # this test is faster than Eppstein's version (checked with timeit)
    return degree_sequence(G) == array('b', [3, 3, 3, 3])  # LABARRE
...
def D3reducible(G, triangleHooks=[], pathHooks=[], finalize=isK4):
    ...
    def triangle(u, v, w):
        ...
        Nu = other_neighbor(v, u, w)
        Nv = other_neighbor(u, v, w)
        Nw = other_neighbor(u, w, v)
        ...
        # Make the change!
        # del G[u],G[v],G[w]
        G.remove_nodes_from((u, v, w))  # LABARRE

        # G[Nu].remove(u)
        # G[Nv].remove(v)
        # G[Nw].remove(w)
        G.remove_edges_from(((Nu, u), (Nv, v), (Nw, w)))  # LABARRE
        ...
        # G[Nv].add(x)
        # G[Nw].add(x)
        G.add_edges_from(((Nu, x), (Nv, x), (Nw, x)))  # LABARRE
    ...
    def path(u, v, w):
        ...
        # apexes = G[u] & G[v] & G[w]
        apexes = set.intersection(*map(set, (G[u], G[v], G[w])))  # LABARRE
        if len(apexes) != 1:
            return
        apex = apexes.pop()
        ...
        # Make the change!
        # del G[v]
        G.remove_node(v)  # LABARRE
        # G[u].remove(v)
        # G[w].remove(v)
        # G[apex].remove(v)
        G.remove_edges_from(((u, v), (w, v), (apex, v)))
        # G[u].add(w)
        # G[w].add(u)
        G.add_edge(u, w)  # LABARRE
...
def isOuterK4(G, outer):
    ...
    if not isK4(G):
        return False
    return any(v not in outer for v in G)  # LABARRE
    # for v in G:
    #     if v not in outer:
    #         return True
    # return False
...
def is_wheel(G, triangleHooks=[], pathHooks=[], finalize=isK4):
    """Test if the graph G is a wheel."""

    def no_triangle(G, u, v, w, Nu, Nv, Nw, x): return False

    return D3reducible(G, [no_triangle], pathHooks, finalize)
