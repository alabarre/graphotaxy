...
import unittest
from graph_recognition.LATER.Graphs import *
...
def HamiltonianCycles(G):
    ...
    if not G or not isUndirected(G) or degree_sequence(G)[0] > 3:
        raise ValueError("HamiltonianCycles input must be undirected degree three graph")
    if degree_sequence(G)[-1] < 2:
        return
...
class CubicHamTest(unittest.TestCase):
    ...
    def testSierpinski(self):
        ...
        """
        G = twistedLadder(4)  # Complete graph on four vertices
        for i in range(3):
            G = truncate(G)
            self.check(G, 3)
