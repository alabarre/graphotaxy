...
func enumerateParallel(n int, numWorkers int, output chan *graph.DenseGraph) {
	...
	for i := 0; i < numWorkers; i++ {
		...
		go func() {
			go search.WithPruning(n, c, tmp, numWorkers, func(g *graph.DenseGraph) bool { return false }, func(g *graph.DenseGraph) bool { return !isCircleGraph(g, mat, b, neighbours) })
			for v := range c {
				output <- v
			}
			count <- false
